#!/usr/bin/env python3
"""Combine study2d_*.csv files and add observed_slope, math_level from manifests/Qualtrics."""
...
def main():
    ...
    # Step 1: Combine CSVs
    csv_files = sorted(p for p in script_dir.glob('study2d_*.csv')
                      if p.name != 'study2d_combined.csv')

    if not csv_files:
        print('No study2d_*.csv files found in', script_dir)
        return
    ...
    output_path = script_dir / 'study2d_combined.csv'
    headers = headers + ['observed_slope', 'math_level']

    # Step 2: Load main manifest (study2d uses main manifest for both training and main trials)
