#!/usr/bin/env python3
"""Combine study2b_*.csv files and add observed_slope, math_level from manifests/Qualtrics."""
...
def main():
    ...
    # Step 1: Combine CSVs
    csv_files = sorted(p for p in script_dir.glob('study2b_*.csv')
                      if p.name != 'study2b_combined.csv')

    if not csv_files:
        print('No study2b_*.csv files found in', script_dir)
        return
    ...
    output_path = script_dir / 'study2b_combined.csv'
